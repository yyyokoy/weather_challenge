...
def lr_schedule(epoch):
    ...
    '''
    return 0.001 if epoch < 75 else 0.0001


# starting at this time step, the prediction from the previous time step will be treated as the actual input
nt = 24
extrap_start_time = 25
orig_weights_file = os.path.join(
    WEIGHTS_DIR, 'prednet_sat_weights.hdf5')  # original t+1 weights
...
extrap_weights_file = os.path.join(
    WEIGHTS_DIR, 'prednet_sat_weights-extrapfinetuned.hdf5')
extrap_json_file = os.path.join(
    WEIGHTS_DIR, 'prednet_sat_model-extrapfinetuned.json')
...
prednet = PredNet(
    weights=orig_model.layers[1].get_weights(), **layer_config)
...
train_generator = SequenceGenerator(
    train_file, train_sources, nt, batch_size=batch_size, shuffle=True, output_mode='prediction')
val_generator = SequenceGenerator(
    val_file, val_sources, nt, batch_size=batch_size, N_seq=N_seq_val, output_mode='prediction')
...
if save_model:
    ...
    callbacks.append(ModelCheckpoint(filepath=extrap_weights_file,
